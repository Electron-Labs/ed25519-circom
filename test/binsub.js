...
describe("Binary Subtracter Test", () => {
	describe("when performing binary subtraction on two numbers", () => {
		it("should subtract them correctly when first number is greater than second number", async () => {
			const cir = await wasm_tester(path.join(__dirname, "circuits", "binsub1.circom"));
      ...
      asBits2.push(0n);
			...
			const expected = utils.pad(utils.normalize(utils.buffer2bits(utils.bigIntToLEBuffer(a-b))), 200);
			assert.ok(witness.slice(1, 201).every((u, i) => {
				return u === expected[i];
			}));
  ...
  describe("when performing binary subtraction on two numbers", () => {
      ...
      asBits2.push(0n);
			...
			const expected = utils.pad(utils.normalize(utils.buffer2bits(utils.bigIntToLEBuffer(BigInt(2)**BigInt(200) - (b-a)))), 200);
			assert.ok(witness.slice(1, 201).every((u, i) => {
				return u === expected[i];
			}));
