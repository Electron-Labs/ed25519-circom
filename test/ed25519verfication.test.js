...
describe("ED25519 verifcation test", () => {
	describe("When testing against the RFC test vector", () => {
		it("should verify correctly", async () => {
			const cir = await wasmTester(path.join(__dirname,"circuits","ed25519verification.circom"));
			const pointA = [43933056957747458452560886832567536073542840507013052263144963060608791330050n, 16962727616734173323702303146057009569815335830970791807500022961899349823996n, 1n, 47597536765056690778342994103149503974598380825968728087754575050160026478564n];
			const pointR = [26073464383897998325899031212762184271676052677226679463708862316754828477519n, 20246927599389923510374971105736264637524117420538179767629249587300902801762n, 1n, 7867784340861643381702890578607277776011430152424699121581244161773093676488n];
			...
			try {
				cir.calculateWitness({msg:bitsMsg,A:bitsA,R8:bitsR8,S:bitsS,PointA:chunkA,PointR:chunkR});
			} catch(e) {
				console.log(e);
				assert.ok(false);
			}
